...
- Таблица Calendar:
  - Основной ключ доступа (PRIMARY KEY):
    ...
    - unique_id включен в clustering columns для уникальности
  - Специальные индексы:
    - title (SASI, PREFIX mode) - для поиска календарей по началу названия (только префиксный поиск)
    - unique_id (вторичный индекс) - для быстрого изменения/удаления всех копий (использовать ограниченно)

- Таблица Event:
  - Основной ключ доступа (PRIMARY KEY):
    - (user_id, calendar_id, created_at DESC) - для эффективного поиска событий в календаре по временному интервалу
    - event_id включен в clustering columns для уникальности
  - Специальные индексы:
    - start_date (SASI, SPARSE mode) - для поиска событий в заданном временном диапазоне
    - name (SASI, PREFIX mode) - для быстрого поиска событий по началу названия
    - unique_id (вторичный индекс) - для управления копиями событий
...
Для хранения сессий и ошибок рекомендуется использовать in-memory СУБД, такую как Tarantool, поскольку эта таблица имеет небольшой объем и требуют быстрого доступа к данным.
...
| User                 | Calendar             | Event                 | Session        | Reminder       | Elastic_Search | Invitation_error |
| -------------------- | -------------------- | --------------------- |----------------| -------------- | -------------- |------------------|
| Cassandra            | Cassandra            | Cassandra             | Tarantool      | Cassandra      | Elasticsearch  | Tarantool        |
