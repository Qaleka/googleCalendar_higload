...
Выбранные индексы:
- Таблица Calendar:
  - Основной ключ доступа (PRIMARY KEY):
    - user_id - для быстрого получения календарей пользователя
  - Специальные индексы:
    - title (вторичный индекс) - для точного поиска по полному названию календаря

- Таблица Event:
  - Основной ключ доступа (PRIMARY KEY):
    - (user_id, calendar_id) - для быстрого доступа к событиям в конкретном календаре пользователя
    - event_id включен в clustering columns для уникальности
  - Специальные индексы:
...
- Таблица Reminder:
  - Основной ключ доступа (PRIMARY KEY):
    - (user_id, remind_time ASC) - для эффективного получения ближайших напоминаний
    - reminder_id включен в clustering columns для уникальности
  - Особенности:
...
Для таблиц User, Reminder, Session, Calendar и Event целесообразно использовать Cassandra, так как эта СУБД хорошо справляется с большими нагрузками на чтение и запись.
...
Сессии и ошибки не требуют шардирования из-за их небольшого размера.
...
### Основной запрос системы: получение событий пользователя
Главный запрос системы предназначен для получения списка событий конкретного пользователя за указанный период времени. Запрос выполняется к таблице Event и использует оптимальную схему индексации для обеспечения высокой производительности даже при работе с большими объемами данных.
Основной запрос выглядит следующим образом:
```
SELECT * FROM events 
WHERE user_id = [current_user_id] 
  AND calendar_id = [target_calendar_id]
  AND time_start >= [start_date] 
  AND time_finish <= [end_date]
```
Ключевые особенности реализации:
1. Использование PRIMARY KEY:
   - Запрос использует составной первичный ключ (user_id, calendar_id), что позволяет Cassandra быстро находить все события конкретного пользователя
   - Данные физически группируются на узлах кластера по user_id, обеспечивая минимальную задержку
2. Оптимизация временных запросов:
   - Для эффективного поиска событий в заданном диапазоне используется специальный SASI-индекс в режиме SPARSE для полей time_start и time_finish
   - Индекс оптимизирован именно для диапазонных запросов вида "между датой X и Y"

3. Кеширование результатов:
   - Результаты запроса кешируются в Redis с составным ключом user_events:[user_id]:[date_range]
   - TTL кеша составляет 5 минут, что обеспечивает баланс между актуальностью данных и нагрузкой на БД

### Механизм синхронизации изменений событий
Когда владелец события вносит изменения (например, меняет дату или время), система обеспечивает согласованное обновление всех копий события у участников. Процесс синхронизации реализован следующим образом:
1. Инициирование изменения:
   - Владелец обновляет основную запись события через атомарный запрос:
  ```
UPDATE events 
SET time_start = [new_value],
  time_finish = [new_value],
  version = version + 1
WHERE id = [event_id]
IF version = [expected_version]
```
  - Операция проверяет версию, предотвращая конфликты параллельного редактирования
2. Распространение изменений:
   - Система находит всех участников в поле members
   - Для каждого участника формируется команда обновления и отправляется в Kafka:
```
{
  "type": "EVENT_UPDATE",
  "event_id": "123e4567-e89b-12d3-a456-426614174000",
  "fields": {
    "time_start": "2023-11-20T15:00:00Z",
    "time_finish": "2023-11-20T16:30:00Z"
  },
  "timestamp": "2023-11-15T09:30:45Z"
}
```
3. Обработка у участников:
   - Сервис-консьюмер получает сообщения и выполняет массовое обновление:
```
UPDATE events 
SET time_start = [new_time_start],
    time_finish = [new_time_finish]
WHERE user_id = [participant_id]
```
4. Гарантия доставки:
   - Неудачные попытки обновления повторяются с экспоненциальной задержкой
   - После 3 попыток сообщения попадают в dead-letter queue для анализа
5. Инвалидация кеша:
 - Обновляются кеши всех затронутых пользователей:
```
DEL user_events:[user_id1]
DEL user_events:[user_id2]
```
...
| Redis         | Кэширование                              | База данных в оперативной памяти, которая обеспечивает быстрый доступ к данным. Необходима для хранения сессий, а также для кэширования, так как для этих задач (малый размер и высокая производительность) наиболее эффективна in-memory БД. |
