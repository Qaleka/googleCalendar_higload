...
| Количество записей | 3 млрд  | 25(ограничение на кол-во календарей) * 3 млрд | 365 (сихронизация событий за год) * 3 (среднее число создаваемых событий в день) * 3 млрд | 3 млрд  | 3 млрд * 25 (макс число календарей пользователя) * 2 (среднее число добавленных каледарей) | 365(сихронизация только за год) * 3 (среднее число гостей) * 3 млрд | 365(синхронизация только за год) * 2(среднее число напоминаний) * 3 млрд |
| Общий размер       | 1,44 Тб | 85,67 Тб                                      | 3,54 Пб                                                                                   | 447 Гб  | 3,27 Тб                                                                                    | 224,07 Тб                                                           | 163,32 Тб                                                                |
...
| Event           | 7234(Просмотр календаря)                                             | 4341(Создание события) + 1447(Изменить информацию о событии) = 5788 | 
| Session         | -                                                                    | 5788(Приглашение гостя без аккаунта)                                |  
| Guests_calendar | 7234(Просмотр календаря)                                             | -                                                                   | 
| User_to_event   | -                                                                    | 5788(Приглашение гостя) + 4341(Ответ гостя) = 10129                 | 
| Reminder        | 2894(Получение напоминания)                                          | 4341(Создание события)                                              | 
...
Выбранные индексы:
- Таблица Calendar:
  - Индекс на user_id (hash) для быстрого получения календарей пользователя.
  - Индекс на unique_id (hash) для быстрого изменения и удаления всех копий одного календаря.

- Таблица Event:
  - Индекс на user_id (hash) и creator_id (hash) для быстрого отображения событий пользователя.
  ...
  - Индекс на calendar_id (hash) для отображения всех событий в выбранном календаре.
  - 
- Таблица Reminder:
...
- Таблицы Event и Calendar: Шардирование по полю unique_id позволяет эффективно управлять всеми копиями одного события или календаря.
